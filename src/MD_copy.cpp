...
double Potential() {
        ...
        for (i=0; i<N; i++) {
            for (j=i+1; j<N; j++) {
                rij2=0.;
                for (k=0; k<3; k++) {
                    rij2 += (r[i][k]-r[j][k])*(r[i][k]-r[j][k]);
                }
                
                
                double rij2inv = 1. / rij2;
...
void computeAccelerations() {
    ...
    #pragma omp parallel for num_threads(8) reduction(+:a_copy[:N][:3])
    for (int i = 0; i < N-1; i++) {
        for (int j = i+1; j < N; j++) {
            double rSqd = 0, f;
            double rij[3];
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    #pragma omp parallel for shared(r, v, a) num_threads(8)
    for (int i=0; i<N; i++) {
        for (int j=0; j<3; j++) {
    ...
    // Elastic walls
    #pragma omp parallel for shared(v, a) reduction(+:psum) num_threads(8)
    for (int i=0; i<N; i++) {
        for (int j=0; j<3; j++) {
            v[i][j] += 0.5*a[i][j]*dt;
            if (r[i][j]<0. || r[i][j]>=L) {
